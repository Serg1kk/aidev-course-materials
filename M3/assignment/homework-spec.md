...
#### Шаг 1.1 — Перенос `features.json` в проект

Скачай `features.json` из материалов курса и положи в свой проект. Промпт агенту:

```
Открой aidev-course-materials/M3/project-data/features.json (или возьми
его по ссылке из репо курса). Положи этот файл в наш proshop_mern проект
как backend/features.json (или другая подходящая папка backend'а — на
твой выбор, отрази в README). Это будет live runtime файл для
feature-flags MCP сервера.
```

#### Шаг 1.2 — Создать MCP-сервер с 3 tools
...
```
Создай MCP-сервер с тремя обязательными tools для управления feature flags:
...
Java, Go, .NET). Файл — backend/features.json (live runtime, его сервер
и мутирует).
```

#### Шаг 1.3 — Добавить рекомендуемый 4-й tool `list_features`

```
Добавь в MCP-сервер четвёртый tool: list_features() возвращает все фичи
списком (name, status, traffic_percentage). Без него ты при запросах
про "все фичи" будешь grep'ать features.json напрямую — зачем, если
есть MCP. С этим tool диалог чище и короче.
```

#### Шаг 1.4 — Подключить MCP к IDE
...
#### Шаг 1.5 — Создать страничку `Dashboard Features` в админке

На G1 я делал её одним промптом в навигации, но для HW правильнее в админку:

```
Создай тестовую страничку "Dashboard Features" в админке проекта
(доступ только админ-юзеру после логина). На странице — список всех
фич из backend/features.json с их status и traffic_percentage в виде
таблицы. Источник — backend/features.json напрямую (читать на каждый
рендер).

Опционально для красоты — возьми дополнительный контекст о фичах
(описание, бизнес-смысл, метрики) из aidev-course-materials/M3/
project-data/features-analysis-ru.md и подмешай в таблицу
(колонка "Описание" или tooltip).

В M4 мы будем делать UI-дизайн этой страницы — пока функционал
минимальный, без стилей и фильтров.
```

#### Шаг 1.6 — Пройти тестовый сценарий + автолог в `report.md`
