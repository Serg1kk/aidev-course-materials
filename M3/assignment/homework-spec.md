...
### Обновить rules-файл проекта (CLAUDE.md / AGENTS.md / Cursor rules)

После того как два MCP подключены — **обновите rules-файл вашего проекта** (`CLAUDE.md` для Claude Code, `AGENTS.md` для Codex / OpenCode / Cursor, `.cursor/rules/*.mdc` для Cursor) с инструкцией приоритета поиска. На G1 я делал это одним промптом в конце сессии — агент сам дописал в `CLAUDE.md` секцию вроде:

```markdown
## Поиск по документации продукта proshop_mern

При любых вопросах про функционал, фичи, архитектуру, ADR, runbooks или
incidents продукта proshop_mern:

1. **СНАЧАЛА** — `search_project_docs` MCP (vector DB). Это быстрее и
   возвращает релевантные чанки с метаданными.
2. **ТОЛЬКО если** vector search не дал нужных результатов или нужно
   полное содержимое файла из метаданных найденного чанка → читать файл
   напрямую через grep+read.

Не начинай с grep+read по проекту — это медленно и дорого по токенам.
```

Точную формулировку отдай агенту: «Обнови мой `CLAUDE.md` (или `AGENTS.md` / Cursor rules) — добавь правило: на вопросы про продукт `proshop_mern` сначала использовать search-docs MCP, и только если там не нашлось — fallback на grep+read файлов». Агент сам выберет правильный файл и формулировку под вашу IDE.
