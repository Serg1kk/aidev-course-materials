...
**Создайте тестовую страничку `Dashboard Features` в админке проекта** — простая страница со списком всех фич + текущий статус и трафик. На уроке G1 я делал её одним промптом «положи сверху страничку в админку, выведи список фич и их состояние». В **M4** мы будем делать UI-дизайн этой страницы — пока функционал минимальный (читает `features.json` + рендерит таблицу).
...
В корне репо у вас уже есть `report.md` после M2. Допишите туда секцию `## M3` с логами обоих MCP. **Не пишите эти логи руками** — попросите своего агента (Claude Code, Cursor, Codex, Cline, OpenCode — любой) сам их сохранить. Готовые промпты:

#### Промпт 1 — feature-flags MCP

```
Возьми любую фичу из features.json (на твой выбор). Через feature-flags MCP:
1. Узнай её текущее состояние.
2. Поменяй статус (Disabled→Testing или Testing→Enabled — что подходит).
3. Скорректируй traffic_percentage.
4. Верни через MCP финальное состояние для подтверждения.

После прогона — допиши в report.md в секцию ## M3 → ### Feature flags MCP
полный лог: какую фичу выбрал, какие tool calls сделал с аргументами,
что MCP возвращал на каждом шаге, итоговое состояние фичи.
Markdown, чистым текстом, без скриншотов.
```

#### Промпт 2 — search-docs MCP
