...
**Pixel-Perfect цикл:**
Паттерн из практики Константина Доронина (Telegram @kdoronin_blog). Замыкает feedback loop который обычно делает дизайнер руками — сравнивает скриншот рендера с макетом. Автоматизация: Browser MCP делает скриншот рабочего приложения, Figma MCP — скриншот из дизайна, vision модель находит отличия, агент фиксит код, цикл повторяется.

Аналогичный подход описывает Leon van Zyl (YouTube канал Leon van Zyl): UI reviewer subagent как часть production workflow после каждого значительного изменения UI.
...
- Leon van Zyl, "UI reviewer subagent" (YouTube канал Leon van Zyl, 2026) — паттерн review/enforce режимов, `.claude/agents/ui-expert.md` шаблон. Видео `gfmbH7aU3TU`, KB hit RANK 3.
- Константин Доронин (Telegram @kdoronin_blog) — Pixel-Perfect verification cycle: Browser MCP + Figma MCP + vision diff. https://t.me/kdoronin_blog/602

Детали в KB research: `lessons/lesson4/materials/kb-research/query2-frontend-agents-workflows/findings.md`, раздел 4 "Уровни верификации".
...
- **Уровень 5:** UI reviewer subagent (Leon van Zyl) — review + enforce режимы
- **Уровень 2:** Pixel-Perfect subagent (Константин Доронин) — Browser MCP + Figma MCP + vision diff
