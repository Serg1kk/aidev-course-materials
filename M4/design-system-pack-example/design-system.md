...
ZIP-package паттерн — Alexey TripleA (Алексей Андреевский), серия «Vibe SaaS», эпизоды #7-#8 (январь 2026). Ключевая идея: дизайн-система как версионируемая структура данных в БД, экспортируемая в три формата (JSON + CSS + MD) → ZIP для AI-агентов.

YouTube канал: [https://www.youtube.com/@AlexeyTripleA](https://www.youtube.com/@AlexeyTripleA)

> «Все эти файлы, которые мы экспортируем — это в первую очередь референсы, по которым нейронка должна работать, а не готовый код. Но как референс, они должны хорошо работать.»
>
> — Alexey TripleA, эпизод #8
